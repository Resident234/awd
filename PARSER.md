...
6. Ошибка одной ссылки (404, login-required, таймаут, битый HTML) не останавливает проход — логируется и учитывается в счётчиках
...
- `vendor/bin/codecept run Unit` — 32 теста зелёные, включая нормализацию «Вчера»/«Сегодня», статистику сервиса и блокировку
