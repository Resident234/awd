...
#### Settings Module (`app/shared/Settings`)
- `PublicationSettingsService` - The tunables of the publications page and the form that edits them: one `SCHEMA` entry per tunable (label, type, bounds, unit, hint, default), reading that always returns a usable value, writing that refuses a value that does not fit
- `PublicationSettingsRepositoryInterface` / `PublicationSettingsRepository` - One row per tunable in `publications_setting`
...
- `SiteController` (`app/controllers/SiteController.php`) - Web UI controllers for publications, channel management and the two settings pages (`settings`, `parser-settings`)
...
The publications feature uses four tables to manage the lifecycle of content, plus one table to hold its tunables:
...
| `publications_setting` | Tunables of the publications page | - one row per code (`code` → `value`, `updated_at`)<br>- seeded from `PublicationSettingsService::defaults()` by `m260925_000001_create_publications_setting` |

A publication whose text goes past the 4096-character limit of one Telegram message is stored as several `publications_post` (or `publications_draft`) rows, one per part: the rows are written by a single `createPosts()`/`createDrafts()` call inside a transaction, `published_at` puts each part the configured gap after the previous one (`partsOffsetMinutes`, one minute by default, never shorter than the `publish-due` schedule), and there is no group column — the parts are ordinary records, ordered by `id`, and the «Часть N» heading is part of the text itself. Each row keeps its own `image_urls`, so a part is sent to the channel with the photos it holds: the form gives every part a field of its own (`publicationPartImages[]`, the album of the first part being the shared `publicationImages` field), and what a field holds on submit is what that row gets — an album that belongs to no part of its own stays with the first one. Beside the caret split the form offers three manual modes for the text as a whole — by paragraphs, by lines, by sentences (`splitWholeText`, the buttons carrying `data-split-whole`): one part per boundary of the chosen kind, however short it comes out, with a piece too long for one message still cut down by `splitIntoParts`. Two buttons under each part's album field move the whole list to the neighbouring part (`moveImageGroup`, `data-move-album`): the received links come in behind the ones the receiving field already held — `unionGroups`, so a link both fields showed is named once — and the field they left stands empty, while the last part has no «next» button and moving an empty list is refused so a stray click cannot empty the neighbour's field. See «Разбивка длинного текста на части» in [README.md](README.md). Every album field also carries a file picker of its own — «Файлы публикации» / «Файлы этой части» (`publicationImageFiles[]` for the shared field, `publicationPartImageFiles{N-1}[]` for part *N*, the first block's picker hidden and disabled exactly like its links field): a file chosen there is stored under the web root and becomes one more link of that same album, so `image_urls` stays a list of links and neither the record nor the channel has to know anything about the upload. See «Файлы альбома» in [README.md](README.md).
...
- **`PublicationRepositoryInterface`** - Abstract repository used by the service to persist publications (implemented by `PublicationRepository`, which keeps all the SQL):
  ...
  - `createPosts(array $parts, string $now): int` - Inserts the parts of one long publication as separate posts **in a single transaction** (all rows or none); `$parts` is a list of `['text' => …, 'imageUrls' => …]` rows, so every part carries its own album. Returns the id of the first part
...
| `saveParts(array $texts, array $imageGroups, string $publishedAt, string $action, ?ForumPublicationRef $forumRef = null, ?string $userTimezone = null): void` | Entry point of the publication form for a **new** record, which sends one text field and one album field per part of a long publication. Every part goes through the text validation before anything is written. A single field is saved exactly as `saveDraft()`/`schedulePost()` would. Several fields become one record each through a single repository call (`createPosts()`/`createDrafts()`), so a failure halfway leaves none of the parts behind; `$action` (`'draft'` or `'publish'`) picks the table, the forum link belongs to the first record, and its `published_at` is the form date with each following part shifted by the configured parts gap (`partsOffsetMinutes`, read through `PublicationSettingsService::intValue()` and floored by the `publish-due` cron interval), so `publishDue()` drains the parts in order. `$imageGroups` is read positionally against `$texts` — the album of part *n* is the list the form held in field *n* — and a part whose field stands empty is saved without images. |
...
Three more variables left the source code and are read from the environment:

```
# Signs the session and CSRF cookies (app/config/web.php); empty key = no request
# carrying one of them is accepted. Generate: php -r "echo bin2hex(random_bytes(32));"
COOKIE_VALIDATION_KEY=
# Both are read by app/web/index.php and app/yii; without them the app is production
YII_ENV=dev
YII_DEBUG=1
```

None of the three is displayed on a settings page: a page that lists tunables is not where a secret belongs.

---

## Usage from the Web UI

The **SiteController** (`app/controllers/SiteController.php`) provides a UI for managing the channel description and for viewing the connection status. The dashboard (`views/site/index.php`) shows whether the channel is reachable. Two more pages hold the tunables: `/settings` («Настройки публикаций») edits every entry of `publications_setting` through `PublicationSettingsService::save()`, shows the Telegram protocol limits and the `publish-due` schedule read-only, and keeps the channel description cards it inherited from the old `/channel-settings` page; `/parser-settings` («Настройки парсера») edits the HTTP behaviour and the entity ranges stored in `parser_config` (see «Настройки парсера» in [PARSER.md](PARSER.md)). Both forms answer a bad value with a flash and store nothing. The rest of the portal reads its numbers from the same place: the page sizes, the scroll edge, the image probe timeout, the manual split snap range, the album preview limits, the size and the number of the files an album accepts from a computer, the schedule picker step and horizon, the ceiling of the forum image-count filter, the default reading order of the lists, the date format and the gap between the parts of a publication (`SiteController::publicationSettings()`, injected into `app/views/site/publications.php` as `__*` globals and into the widgets through `PublicationsUi`). See «Страницы настроек» in [README.md](README.md).

The publications page (`/publications`) and its form posts to `publication-create`, which calls `PublicationsService::saveParts` for a new record and `saveFromForm` for a record opened for editing; the per-record buttons call `publishDraft`/`publishPostNow`, `movePostToDraft` and `deletePost`/`deleteDraft` through `publication-publish`, `publication-to-draft` and `publication-delete`. The image links a fill brings in (`data-image-urls`, from the record or from the forum) are probed in the browser before they settle in the album field of the part they belong to (`publicationImages` for the first part, `publicationPartImages[]` for the rest): each list goes into its field at once so a submit made mid-probe loses nothing, only links that answer a load are kept, a link that never answers is kept too, and the ones dropped are named in the notice under that same field (see «Открытие на редактирование» in [README.md](README.md)). The files the album pickers brought in are read before the record is written (`SiteController::withUploadedImages`): every album's files are looked up by the name of its picker — `UploadedFile::getInstancesByName()`, so an empty picker of one part cannot shift the files of another — and each one is checked against the two upload settings, against `getimagesize()` for the format and for having arrived whole, before any of them leaves the temporary directory. A file that passes becomes `web/uploads/publications/YYYYMM/<random>.<ext>` served by nginx, and its public URL is appended to the links of that album; anything refused raises `InvalidArgumentException`, which the form answers with a flash and without writing a record. A request whose body PHP discarded for being larger than `post_max_size` carries neither fields nor files, so the action notices the empty body against `CONTENT_LENGTH` and explains the size instead of reporting an empty text. See «Файлы альбома» in [README.md](README.md). The three record lists start at one page of `publicationPageSize` rows and ask `publication-page` (`posts()`/`drafts()`/`deleted()` with a page and an offset, rendered through the `_item_*` partials) for the next page whenever a block is scrolled to its bottom. The forum block pages on two levels of its own: its topics come from the same `publication-page` endpoint (`ForumRepository::latestTopicsWithPosts` with an offset, `countTopics` for the total), while the posts of one topic sit in a scrollable box of their own (`_item_topic`, `.thread-replies`) and are loaded by `forum-post-page` (`ForumRepository::topicPosts`) as that box is scrolled — see «Бесконечная прогрузка списков» in [README.md](README.md). Beside the «Опубликовать» button of a topic, two more buttons load the whole discussion into the form: they post to `forum-thread`, which answers every post the table holds for that topic (`ForumRepository::topicThread` — no page, no filter, published posts included, `login_required = FALSE` kept) in the order the block currently shows them, while the topic itself is read off the attributes of its own publish button; «Тред целиком» joins the texts into one field and the links of the whole discussion into the album of the first part, «Тред по частям» gives each entity a part of its own with that entity's links in that part's album field, cutting every text with `splitIntoParts` so no part can exceed the limit `saveParts` checks (see «Тред топика в форму» in [README.md](README.md)). A switch in the header of a block posts to `publication-sort`, which stores the reading order of that list in the session and answers with the block the switch belongs to redrawn from its first page in the new order — the forum block has two such switches, one for its topics (`ForumRepository::latestTopicsWithPosts` topic order) and one for the posts of each topic; the session state — the forum filters and the reversed lists alike — is mirrored into the address bar (`SiteController::publicationsUrl`, `?postsOldest=1` and friends), and a page opened on an address that disagrees with the session is redirected onto it (see «Сортировка списков» in [README.md](README.md)). Addresses are flat: `UrlManager` maps each of these routes onto its own path without `index.php?r=` (see «Адреса страниц» in [README.md](README.md)). After a scheduled post becomes due the background `publish-due` command sends it to Telegram.
...
The service use-cases themselves (`saveParts`, `saveFromForm`, the periodic tasks) have no automated test coverage, and no test exercises the publications page in a browser. Neither has a Codeception test been written for the settings: `PublicationSettingsService` (clamping on read, refusal on write, the cron-derived floor) and both settings pages are covered by offline harnesses only, so a migration run against PostgreSQL and a click through the two forms remain to be done.
